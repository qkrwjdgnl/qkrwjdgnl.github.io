...
그러면 이제, 의미를 대표하는 벡터(예를 들어 아래의 파란색 3x5 격자)를 가지게 되었으니, 이 의미 벡터를 실제 이미지 생성기, 즉 아래 그림의 분홍색 Image Generator에 넘겨줍니다.
